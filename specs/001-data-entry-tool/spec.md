...
The tool serves a person who maintains structured tabular data. They open one or more data files alongside a dictionary that describes the data's shape, correct or extend the data in a familiar spreadsheet, and save it back so the same files can be reopened and edited again later. The dictionary is the single source of truth: it decides which tables and columns exist, how each cell behaves, what counts as valid, and how the saved files are structured — so the data, the validation, and the on-disk files can never drift apart.
...
- **Foreign key present**: a foreign-key constraint is shown as informational only and is not enforced in v1.
...
- **Large datasets**: the grid remains responsive when a table has a large number of rows.
...
Requirements are grouped by feature area. FR numbers are stable identifiers (referenced from the design artifacts and the source) and are not sequential within a section.
...
- **FR-002**: The tool MUST interpret the dictionary as the single source of truth for which tables and columns exist, each column's type and constraints, how each cell is presented and validated, and how the saved files are structured.
- **FR-003**: Before opening a table's grid, the tool MUST reconcile its data file against the dictionary by comparing the column set and each column's type.
...
- **FR-011**: For enum columns with keys + values, the tool MUST display the human-readable label to the user, and MUST store/save the underlying key.
...
- **FR-048**: Numeric columns MUST be right-aligned — values in a numeric (quantity) column are shown with a consistent number of decimal places so the decimal points line up.
- **FR-032**: Enum columns MUST offer a selection control (dropdown) listing the allowed labels; the stored value remains the key.
...
- **FR-045**: Users MUST be able to insert a row above or below any row, and delete a row, via a row context menu (right-click); these MUST be undoable. - **FR-046**: Inserting or deleting a row MUST keep validation correct by re-evaluating after the row indices shift.
...
- **FR-036**: The outstanding-violations indicator MUST be actionable: activating it MUST move the selection to (and scroll to) the first violating cell, and activating it again MUST advance to each subsequent violation in order, cycling back to the first after the last.
...
- Keyboard navigation MUST NOT move into a non-existent row beyond the data.
...
- **FR-034**: The tool MUST provide a full undo/redo stack covering all data mutations (cell edits, row additions, paste, fill-down, row insert/delete, delete-to-null), reachable by keyboard (⌘/Ctrl+Z, ⇧⌘/Ctrl+Z or Ctrl+Y) and toolbar controls.

#### Frozen panes and scrolling
...
- **FR-035**: The tool MUST support autosave: where it can overwrite in place, it persists the current state automatically (debounced) after edits, without a confirmation prompt — including flagged-invalid values, since the file legitimately holds in-progress data; the FR-020 warn+confirm applies only to explicit manual saves.
- **FR-047**: Closing a dataset (or a table tab) with unsaved changes or outstanding validation problems MUST prompt for confirmation before discarding.
...
- **Dictionary**: The authoritative description of the data. Describes one or more tables and is the source of truth for tables, columns, types, constraints, presentation, and saved-file structure.
- **Table**: One named table within the dictionary, with its own ordered column definitions, data source, grid, validation, undo history, and save target.
...
- **Dataset**: The collection of records for a table being edited, conforming to the dictionary; loaded from and saved to a data file.
- **Record (Row)**: A single entry in a dataset, one value per column.
...
- **SC-004**: The tool never writes a file with outstanding violations without an explicit user confirmation — 0% of violation-bearing manual saves occur silently.
...
- **SC-006**: The grid remains responsive (scrolling and editing feel immediate, and live validation keeps up) on tables of approximately 10,000 rows.
...
- **Save target**: saving aims to overwrite the original file in place; in a plain browser this may not be reliable, so the tool re-exports the same file. A desktop wrapper (Tauri) provides dependable in-place overwrite, and eventually per-row persistence to disk as data is entered.
- **Scale**: the tool targets tables of roughly 10,000 rows for responsive editing and validation.
- **Editing existing data**: each opened file was previously produced by this tool (or an equivalent producer) and the saved file is intended to be reopened later by this tool. Creating a table from a dictionary alone (no source data file) is out of scope for v1.
- **Both files supplied together**: the user always provides the dictionary alongside the data file(s); the tool does not infer or fetch a dictionary.
...
- **Foreign keys deferred**: foreign-key constraints are displayed but not enforced in v1; cross-table referential checks are a later enhancement.
